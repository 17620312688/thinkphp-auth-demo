...
<script src="/auth_old/Public/Admin/js/base.js"></script>
...
<!--header -->
<div class="header">
	<div class="logo"><div class="logo_img"><img src="/auth_old/Public/Admin/img/logo.png"></div></div>
    <div class="top">	
        <div class="top_link">
        ...
        <div class="menu">
            <ul>
            	<?php if(is_array($data)): foreach($data as $key=>$vo): ?><li><a href="/auth_old/index.php/Admin/<?php echo ($vo["default_name"]); ?>?menu_title=<?php echo (urlencode($vo['title'])); ?> - <?php echo (urlencode($vo['default_title'])); ?>" target="iframe" id="menu_hover<?php echo ($vo["id"]); ?>" onclick="change_menu(<?php echo ($vo["id"]); ?>)"><?php echo ($vo["title"]); ?></a></li><?php endforeach; endif; ?>
            </ul>
        </div>
    </div>
</div>
...
<div class="left" id="left_sub_menu">
    ...
    <?php if(is_array($data)): foreach($data as $key=>$vo): ?><ul class="sub_menu" id="sub_menu<?php echo ($vo['id']); ?>" style="display:none;">
    	<?php if(is_array($vo['sub'])): foreach($vo['sub'] as $k=>$sub): if($k == 0): ?><li><a name="iframe" id="sub_menu_<?php echo ($sub['id']); ?>" onclick="change_sub_menu(<?php echo ($sub['id']); ?>)" href="/auth_old/index.php/Admin/<?php echo ($sub['name']); ?>/menu_title/<?php echo ($vo['title']); ?> - <?php echo (urlencode($sub['title'])); ?>" target="iframe" class="sub_menu_hover"><?php echo ($sub['title']); ?></a></li>
    		<?php else: ?>
    <li><a name="iframe" id="sub_menu_<?php echo ($sub['id']); ?>" onclick="change_sub_menu(<?php echo ($sub['id']); ?>)" href="/auth_old/index.php/Admin/<?php echo ($sub['name']); ?>/menu_title/<?php echo ($vo['title']); ?> - <?php echo (urlencode($sub['title'])); ?>" target="iframe"><?php echo ($sub['title']); ?></a></li><?php endif; endforeach; endif; ?>
